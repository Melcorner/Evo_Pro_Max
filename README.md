...
- Приём webhook событий от Эвотор в нативном формате (`SELL`, `ACCEPT` и др.)
...
- конвертирует цены из рублей в копейки (формат МойСклад)
- устанавливает `syncId = id` документа Эвотор
...
Система принимает события в нативном формате Эвотор:
...
Поддерживаемые типы событий: `SELL` → `sale`.
